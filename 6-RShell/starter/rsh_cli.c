...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            // We will exit but still need to wait for the server's response
            // to confirm that the command was processed
        }
        ...
        while (1) {
            ...
            // Print the response (except for the EOF character if present)
            printf("%.*s", is_eof ? (int)io_size - 1 : (int)io_size, rsp_buff);
            
            // If this is the last chunk, break out of receive loop
            if (is_eof) {
                break;
            }
        }

        // If the sent command was exit or stop-server, we should exit
        if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, OK);
        }
    }
